...
- Human docs: [README.md](README.md) and [docs/README.md](docs/README.md). Version: `packages/ip-service/package.json`; generated `src/lib/version.ts` within that package. Enforcement: `.husky/`, `.github/workflows/ci.yml`, IP-service Vitest configs. Accidents: [Retrospective.md](Retrospective.md). Private configuration: ignored environment files; use variable names only in docs.
...
IP API: Bun, Hono, TypeScript, local MMDB/ip2region files, Vercel production. Collector: Cloudflare Worker with `ECHO` KV binding. DNS probe: Go toolchain from `packages/dns-probe/go.mod`, Docker. Quality: Bun workspaces, Biome, Vitest; Go testing gap below. `packages/ip-service/` owns the API, downloader and tests; `packages/collector/` owns KV collection; `packages/dns-probe/` owns DNS; `swiftbar/` contains the Node 18+ helper.
...
6DQ keeps its name with unified L1, L2/L3, G2 and D1; the owner merged former G1 into L1 on 2026-09-21. Statuses: `enforced`, `planned`, `manual`, `N/A`. No skipped/focused tests; required unified L1 coverage is statements/branches/functions/lines each ≥95% plus strict types and check-only lint with zero errors/warnings.

| Piece | Requirement and current reality | Status | Evidence |
| --- | --- | --- | --- |
| L1 TypeScript | IP-service four metrics ≥95% plus strict types/lint across both packages and helper; Collector and SwiftBar coverage still missing | planned | IP gate enforced in pre-commit/CI and `packages/ip-service/vitest.config.ts`; IP checks and root Biome run as enforced static subchecks, but Collector typecheck is not in CI and gates use the working tree |
| L1 Go | Meaningful DNS coverage plus Go static checks and formatting at the same contract; no suite or gate yet | planned | `packages/dns-probe/`; no Go gate in hooks/CI |
...
Current pre-commit checks the working tree (IP types, lint, coverage) and staged secrets; pre-push runs builder, unit, lint, HTTP and OSV. Target: check-only unified L1 on the index snapshot under 30s; pre-push stdin refs with L2/G2 in parallel under 3min. Commit and branch-push hook bypass is forbidden.
